...
class Perceptron:
  ...
  def __init__(self, x1, x2, x3, outputs, learningRate = 0.1, epochs = 10000):
    self.x1 = x1
    self.x2 = x2
    self.x3 = x3
  ...
  def train(self):
    ...
    while(True):
      ...
      if(globalError == 0 or e >= self.epochs):
        break
    print("Decision linear equation found => {weight1}*x1 + {weight2}*x2 + {weight3}*x3 + {bias} = 0".format(weight1=self.weights[0], weight2=self.weights[1], weight3=self.weights[2], bias=self.bias))
  #endfunction
